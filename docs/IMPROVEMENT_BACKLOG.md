...
| 专注/办公 | 预设、办公室氛围、键盘 | v1.54–1.55 预设覆盖与重命名 |
...
### 代码与架构备注（2026-05-29 第三十五次挖掘）

- **完整备份**：`fullAppBackup.ts` v2 含 `mixerSnapshot` + `appPreferences`；仍兼容 v1 导入；不含活动睡眠/唤醒定时快照（时间敏感）。
- **下一项候选**：Android 后台播放（P1，需原生调研）；新内置环境声（避免连续版本仅加音）。
- **音库**：17 内置轨。
- **测试**：`fullAppBackup.test.ts` 覆盖 v1/v2 往返与偏好写入。
- **版本**：**1.58.0** → 本次 **1.59.0** 完整备份 v2。

### 外部信号

- GitHub Issues：无 open issue（2026-05-29）。
- 同类 App：换机迁移常含「当前播放列表 + 设置」；wix v1.59.0 已对齐单文件 JSON 路径。

## 本次选中项

**完整备份 v2（混音快照 + 应用偏好）（P2）**

- **理由**：v1.53.0 已合并音频与预设，但换机后仍需手动恢复当前混音与主题/渐入/常亮等；单 PR 扩展现有 `fullAppBackup` 模块，无原生依赖，与 v1.58.0 睡眠按时刻无重叠。
- **范围**：`fullAppBackup.ts`、`AppLayout` 导入导出、`SettingsPage` 文案、版本 **1.59.0**。
