...
| **睡眠定时按时刻** | 23:00 自动渐弱 | 仅倒计时分钟 | 对称扩展 `sleepTimer.ts` |
...
| 换机/备份 | 迁移自定义音频 + 场景预设 | v1.53.0 完整备份；快照/偏好仍可选 |
...
### 代码与架构备注（2026-05-29 第三十三次挖掘）

- **唤醒**：`wakeTimer.ts` 支持 `startWakeTimerAtClock` / `resolveWakeEndsAtMs`；与倒计时共用快照与 Web Audio 斜坡。
- **预设/备份**：下一项可为 `fullAppBackup` v2（混音快照+偏好）或睡眠按时刻。
- **音库**：17 内置轨；避免连续版本仅加音。
- **Android**：后台播放仍为 P1（需原生研究）。
- **测试**：`wakeTimer.test.ts`、Studio 抽屉时钟唤醒 UI/读屏测试。
- **版本**：**1.56.0** → 本次 **1.57.0** 时钟点唤醒。

### 外部信号

- GitHub Issues：无 open issue（2026-05-29）。
- 同类 App：固定时刻闹钟常见；wix 唤醒已与竞品对齐「时刻」路径。

## 本次选中项

**时钟点唤醒（P2）**

- **理由**：v1.56.0 已完成定时 Web Audio 斜坡；backlog 明确「7:30 叫醒」缺口；复用 `WakeTimerState` 与持久化，单 PR 无原生依赖。
- **范围**：`wakeTimer.ts`、`useWakeTimerController`、`StudioPage` 抽屉、`playbackAnnouncement`、版本 **1.57.0**。
