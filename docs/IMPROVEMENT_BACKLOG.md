...
| **预设重命名** | 修正错字而不改混音 | 仅删/新建 | 列表项内联编辑或长按菜单 |
...
| 午睡/叫醒 | 倒计时后渐强起播 | v1.41.0 唤醒定时；无时钟点 |
| 冥想/放松 | 自然水声、森林氛围 | v1.50.0 溪流；可与森林/雨叠加 |
| 专注/办公 | 预设、办公室氛围、键盘、快速找轨 | v1.38.0 环境声搜索；v1.54.0 预设可覆盖更新 |
| 哄娃/屏蔽突发声 | 白噪音底噪 | v1.47.0 内置白噪音 |
| 分享/社交 | 发链接好友一键同款混音 | Web/PWA v1.19；Android v1.44–1.45 |
| 换机/备份 | 迁移自定义音频 + 场景预设 | v1.53.0 完整备份；快照/偏好仍可选增强 |
...
### 代码与架构备注（2026-05-29 第三十次挖掘）

- **预设**：`saveMixerPreset` 同名精确匹配（trim 后）覆盖层与主音量，保留 `id`/`createdAt`；满 12 槽仍可刷新已有名称。
- **音库**：17 内置轨；下一加音版本间隔至少一项非音库能力（避免连续纯加音）。
...
- **测试**：`mixerPresets.test.ts` 覆盖覆盖保存与满槽覆盖；无 `src/` 内 TODO。
- **版本**：仓库 **1.53.0** → 本次 **1.54.0** 预设同名覆盖。

### 外部信号

- GitHub Issues：无 open issue（2026-05-29）。
- 同类 App：Android 后台、时钟闹钟仍常见；wix 预设/备份/分享链已较完整。

## 本次选中项

**场景预设同名覆盖保存（P1 专注/睡眠日常）**

- **理由**：12 槽上限下用户常需更新「睡眠」「专注」等同名搭配，删后重存摩擦大；单 PR 改 `mixerPresets.ts` 与状态提示，不碰原生。
- **范围**：`storage/mixerPresets.ts`、`AppLayout` 提示、`StudioPage` 文案、版本 **1.54.0**。
