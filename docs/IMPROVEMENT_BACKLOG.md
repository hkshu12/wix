...
| **新内置环境声（续）** | 雨棚、鸟鸣、风声等 | 已有 17 轨 | 评估 CC0 库 `ambient_02`、`loop_ambient_01/03` 等 |
...
| 换机/备份 | 迁移自定义导入音频 | v1.51.0 设置页 JSON 备份导出/导入 |
...
### 代码与架构备注（2026-05-29 第二十七次挖掘）

- **音库**：17 内置轨；CC0 可续加 `ambient_02`、风声 loop 等（P2）。
- **自定义库**：v1.51.0 新增 `customLibraryBackup` 域模块 + 设置页 UI；备份为 JSON（base64 音频），无 zip 依赖。
- **分享深链**：Web/Android 已齐；混音分享仍不含自定义音轨 blob。
- **定时器**：睡眠/唤醒/锁屏/读屏已对称；Android 后台仍为 P1。
- **测试**：备份 serialize/parse、Settings 导出按钮、storage importStoredCustomTrack。
- **版本**：仓库 **1.50.0** → 本次 **1.51.0** 自定义音轨库备份。
- **近期 CHANGELOG**：v1.50.0 溪流——避免连续两版仅加内置音。

### 外部信号

- GitHub Issues：无 open issue（2026-05-29）。
- 同类 App：Android 后台 + 大音库 + 换机备份仍常见；wix 混音分享已强，自定义库备份补齐换机场景。

## 本次选中项

**自定义音轨库导出/导入备份（P2 → 换机/重装场景，用户可感知）**

- **理由**：backlog 明确列出「自定义音轨库导出」；v1.50.0 刚完成溪流，连续加 CC0 音价值递减；备份比 Android 后台（P1、需原生研究）更适合单 PR；设置页已有「数据与存储」与清除数据，自然延伸。
- **范围**：`customLibraryBackup.ts`、storage 读写 bytes、`SettingsPage` 导出/导入 UI、版本 **1.51.0**。
