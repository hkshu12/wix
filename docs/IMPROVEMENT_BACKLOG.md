...
| ~~新内置环境声（办公室 HVAC）~~ | 办公室空调、专注底噪 | 已有 13 轨 | 已完成 v1.36.0 |
| **新内置环境声（续）** | 工地、白噪音等 | 已有 14 轨 | 工地 `loop_construction_site` CC0 |
...
| **音轨搜索/筛选** | 14+ 内置 + 多自定义 | 平铺网格 | 抽屉内按标题过滤 |
...
| 专注/办公 | 预设、办公室氛围、键盘控制 | v1.36.0 办公室环境声 |
...
### 代码与架构备注（2026-05-28 第十四次挖掘）

- **内置声库**：14 轨（v1.36.0 新增办公室）；下一内容向：工地环境声（P2）。
- **Android**：仍无 foreground service（P1）；分享深链为 P2。
- **落地页**：已更新为十四种内置环境声；仍缺 fade-in、wake lock、clear data 等 v1.28–1.35 功能描述。
- **版本**：当前实现目标 **1.36.0**；下一项宜 **键盘主音量读屏**、**唤醒定时** 或 **Android 分享深链**。

### 外部信号

- GitHub Issues：无 open issue（2026-05-28）。
- 近期 CHANGELOG：v1.35.0 AudioContext 可见性恢复——**避免重复**。
- 同类 App：原生后台、唤醒闹钟、音轨搜索仍常见缺口。

## 本次选中项

**内置办公室环境声（P2 → CC0 `loop_machine_01`）**

- **理由**：backlog 明确列为下一内容向；办公/远程专注是高频场景；与咖啡馆、风扇形成互补；单 PR、无原生依赖、沿用既有 CC0 下载与 catalog 模式。
- **范围**：`office.ogg` + `sounds.ts` + 下载脚本/署名 + 落地页文案 + vitest；版本 **1.36.0**。

## 历史已完成

| 日期 | 项 | 引用 |
| --- | --- | --- |
| 2026-05-28 | 内置办公室环境声 | v1.36.0（本次） |
