...
| **新内置环境声（续）** | 雨棚、鸟鸣等 | 已有 17 轨 | 评估 CC0 库中其他 loop |
...
### 代码与架构备注（2026-05-29 第二十六次挖掘）

- **音库**：17 内置轨（雨/海边/溪流三分水声）；分享链接仍不含自定义音轨 blob；CC0 可续加雨棚等（P2）。
- **分享深链**：`useMixerShareDeepLink` 单测已覆盖 Web/PWA；Android 有 `useAndroidMixerShareDeepLink`。
- **定时器**：睡眠/唤醒/锁屏/读屏已对称；缺时钟闹钟（非倒计时）与 Android 后台（P1）。
- **测试**：深链 hook 与 Studio 定时 UI 回归齐全；下一候选为自定义库导出或新 CC0 环境声。
- **版本**：仓库 **1.49.0** → 本次 **1.50.0** 内置溪流。
- **近期 CHANGELOG**：v1.49 分享深链单测——避免重复测试-only PR。
...
**内置溪流环境声（P2 → 冥想/放松场景，用户可感知）**

- **理由**：backlog 明确列出溪流；CC0 `loop_water_01` 与海边（`loop_water_02`）、雨声差异化；单 PR 可完成（音轨 + 落地页 + 测试 + 归因），比 Android 后台或库导出更小、更稳。
- **范围**：`stream.ogg`、`sounds.ts`、下载脚本、落地页十七种、版本 **1.50.0**。
