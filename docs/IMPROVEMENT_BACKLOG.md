...
| **新内置环境声（续）** | 雨棚、溪流等 | 已有 16 轨 | 评估 CC0 库中其他 loop |
...
| **Studio 唤醒定时 UI 测试** | 回归唤醒流程 | 仅有睡眠定时用例 | 镜像 `StudioPage` 睡眠定时测试 |
...
### 代码与架构备注（2026-05-29 第二十三次挖掘）

- **音库**：16 内置轨（棕/粉/白噪音齐全）；分享链接仍不含自定义音轨 blob。
- **定时器**：睡眠/唤醒/锁屏/读屏已对称；缺时钟闹钟（非倒计时）与 Android 后台。
- **测试**：`StudioPage` 缺唤醒定时 UI 用例；`useMixerShareDeepLink` 无单测。
- **版本**：仓库 **1.46.0** → 本次 **1.47.0** 内置白噪音。
- **近期 CHANGELOG**：v1.46 锁屏唤醒副标题——避免重复。

### 外部信号

- GitHub Issues：无 open issue（2026-05-29）。
- 同类 App：Android 后台 + 大音库 + 时钟闹钟仍常见；wix Web/PWA 能力已齐，原生后台为最大差距。

## 本次选中项

**内置白噪音环境声（P2 → 专注/哄娃/屏蔽突发噪音）**

- **理由**：backlog 标注「无独立白噪音轨」；CC0 库已有 `noise_03.ogg`，与粉/棕噪音同管线，单 PR 可完成（域定义 + OGG + 落地页 + 测试），无需 Android Gradle。
- **范围**：`sounds.ts`、`public/sounds/white-noise.ogg`、下载脚本、ATTRIBUTION、落地页；版本 **1.47.0**。
