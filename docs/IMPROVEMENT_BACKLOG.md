...
| 专注/办公 | 预设、办公室氛围、键盘控制 | v1.36.0 办公室；v1.37.0 +/- 读屏 |
| 多任务 | 切标签/后台后仍听到混音 | v1.35.0 Web Audio 可见性恢复 |
| 隐私/换机 | 一键清除本机数据 | v1.31.0 设置页两步确认 |
| 弱网 | 内置 OGG 偶发失败 | 自动重试 + 按轨重试 v1.28.0 |
| 读屏 | 播放/加轨/定时/键盘音量 | v1.37.0 +/- 主音量播报 |
| Android 原生 | 后台播放 | 仍为 P1 最大缺口 |

### 代码与架构备注（2026-05-28 第十五次挖掘）

- **读屏**：`playbackAnnouncement` 已覆盖播放、加轨、睡眠定时、键盘主音量；滑块拖动主音量仍静默（可后续补）。
- **内置声库**：14 轨；下一内容向：工地环境声（P2）。
- **Android**：仍无 foreground service（P1）；分享深链为 P2。
- **落地页**：仍缺 fade-in、wake lock、clear data、键盘音量读屏等 v1.28–1.37 功能描述。
- **版本**：当前实现目标 **1.37.0**；下一项宜 **音轨搜索/筛选**、**唤醒定时** 或 **工地环境声**。

### 外部信号

- GitHub Issues：无 open issue（2026-05-28）。
- 近期 CHANGELOG：v1.36.0 办公室环境声——**避免重复**。
...
- **理由**：v1.21.0 已支持 +/- 调主音量但读屏静默；与 v1.34.0 睡眠定时播报同一 `aria-live` 区域，改动小、可测、直接改善低视力/读屏用户在 Studio 免开抽屉调音量。
- **范围**：`formatMasterVolumeAnnouncement` + `StudioPage` 快捷键回调 + vitest；版本 **1.37.0**。

## 历史已完成

| 日期 | 项 | 引用 |
| --- | --- | --- |
| 2026-05-28 | 键盘主音量读屏播报 | v1.37.0（本次） |
