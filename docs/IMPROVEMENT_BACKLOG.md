...
| **新内置环境声（续）** | 办公室空调、工地等 | 已有 13 轨 | 办公室 HVAC / `loop_machine` CC0 |
| ~~大文件导入进度~~ | 长播客/长环境录音 | — | v1.16.0 |
| ~~混音 ARIA 实时区域~~ | 读屏知播放/定时状态 | — | 已完成 v1.12.0 |
| ~~落地页功能列表更新~~ | 新用户了解分享链接、快捷键等 | — | v1.27.0 |
...
| ~~AudioContext 后台恢复~~ | 切回标签页后无声 | 仅 play 时 resume | `visibilitychange` → resume + sync v1.35.0 |
| **自定义音轨库导出** | 换机备份导入的音频 | 仅 IndexedDB 存取 | 导出 zip 或单文件 |
| **唤醒定时** | 早晨渐强叫醒 | 无 | 定时主音量渐强 + 可选起播 |
| **Android 分享深链** | APK 打开 `/studio?share=…` | 无 intent-filter | `App.addListener('appUrlOpen')` + manifest |
| **键盘主音量读屏播报** | 低视力用户 +/- 调音量 | 静默更新 | 扩展 `playbackAnnouncement` |
| **音轨搜索/筛选** | 13+ 内置 + 多自定义 | 平铺网格 | 抽屉内按标题过滤 |

### 体验场景与缺口（摘要）

| 场景 | 典型需求 | 现状 |
| --- | --- | --- |
| 睡眠 | 定时、渐出、渐入、读屏、常亮 | v1.34.0 定时播报 |
...
| 读屏 | 播放/加轨/定时 | v1.12.0 + v1.34.0 |
| Android 原生 | 后台播放 | 仍为 P1 最大缺口 |

### 代码与架构备注（2026-05-28 第十三次挖掘）

- **可见性恢复**：`useAudioContextResume` 监听 `visibilitychange`；`AppLayout` 在播放中回到前台时 `resume()` + `sync()`。
- **内置声库**：13 轨；下一内容向：办公室 HVAC（P2）。
- **Android**：仍无 foreground service（P1）；分享深链为 P2。
- **版本**：当前实现目标 **1.35.0**；下一项宜 **办公室环境声**、**唤醒定时** 或 **Android 分享深链**。

### 外部信号

- GitHub Issues：无 open issue（2026-05-28）。
- 近期 CHANGELOG：v1.34.0 睡眠定时读屏——**避免重复**。
- 同类 App：原生后台、办公室白噪、唤醒闹钟仍常见缺口。

## 本次选中项

**AudioContext 可见性恢复（P2 → `useAudioContextResume`）**

- **理由**：用户切标签或后台 App 后 UI 仍显示播放，但 Web Audio 被浏览器挂起导致无声；单 PR、无原生依赖、与 `useScreenWakeLock` 同模式；修复真实跨平台体验 bug。
- **范围**：`useAudioContextResume` hook + `AppLayout` 接线 + vitest；版本 **1.35.0**。

## 历史已完成

| 日期 | 项 | 引用 |
| --- | --- | --- |
| 2026-05-28 | AudioContext 可见性恢复 | v1.35.0（本次） |
