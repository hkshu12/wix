...
| 换机/备份 | 迁移自定义音频 + 场景预设 | v1.53.0 完整备份单文件；仍保留分项备份 |
...
### 代码与架构备注（2026-05-29 第二十九次挖掘）

- **音库**：17 内置轨；CC0 可续加风声 `ambient_02` 等（P2，避免连续纯加音版本）。
- **备份**：v1.53 完整备份 `fullAppBackup.ts`；分项 JSON 仍保留兼容旧文件。
- **分享深链**：仍不含自定义音轨 blob；完整备份可迁移预设引用的自定义轨。
- **定时器**：睡眠/唤醒/锁屏/读屏已对称；Android 后台仍为 P1。
- **测试**：`fullAppBackup` 单测 + Settings 完整备份按钮；可补 file-input 导入 E2E。
- **版本**：仓库 **1.52.0** → 本次 **1.53.0** 完整备份。
- **近期 CHANGELOG**：v1.52.0 场景预设——本次单文件换机闭环。
