...
| **新内置环境声（续）** | 白噪音等 | 已有 15 轨 | 评估 CC0 库中其他 loop |
| ~~大文件导入进度~~ | 长播客/长环境录音 | — | v1.16.0 |
| ~~混音 ARIA 实时区域~~ | 读屏知播放/定时状态 | — | 已完成 v1.12.0 |
| ~~落地页功能列表更新~~ | 新用户了解分享链接、快捷键等 | — | v1.39.0 同步至 v1.38 能力 |
...
| **主音量滑块读屏播报** | 拖滑块调主音量 | 仅键盘 +/- 有播报 | `onChange` 节流 announce |
...
| 读屏 | 播放/加轨/定时/键盘音量 | 滑块拖主音量仍静默（P2） |
| Android 原生 | 后台播放 | 仍为 P1 最大缺口 |

### 代码与架构备注（2026-05-28 第十七次挖掘）

- **内置声库**：15 轨（含工地 `construction-site`）；CC0 源 `sfx100v2_loop_construction_site.ogg`。
- **Studio**：`filterSoundsByQuery` + 搜索框稳定；下一体验向：唤醒定时或主音量滑块读屏。
- **Android**：仍无 foreground service（P1）；分享深链为 P2。
- **落地页**：已与 v1.38–1.39 能力对齐（十五种内置、搜索、渐入/常亮/读屏音量）。
- **版本**：当前实现目标 **1.39.0**；下一项宜 **唤醒定时**（P2，域逻辑可复用 sleep timer）或 **主音量滑块读屏**（小、a11y）。

### 外部信号

- GitHub Issues：无 open issue（2026-05-28）。
- 近期 CHANGELOG：v1.38.0 Studio 搜索——**避免重复**。
- 同类 App：原生后台、唤醒闹钟仍常见缺口。

## 本次选中项

**内置工地环境声（P2）+ 落地页能力同步**

- **理由**：backlog 已标注 CC0 源路径；与办公室/公路等同模式，单 PR 可完成；居家装修、城市氛围是明确场景；顺带更新落地页「十四种」等过时文案。
- **范围**：`construction-site.ogg`、`sounds.ts`、下载脚本、归因、落地页与测试；版本 **1.39.0**。

## 历史已完成

| 日期 | 项 | 引用 |
| --- | --- | --- |
| 2026-05-28 | 内置工地环境声 + 落地页同步 | v1.39.0（本次） |
