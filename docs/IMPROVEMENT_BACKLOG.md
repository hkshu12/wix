...
| 换机/备份 | 迁移自定义音频 + 场景预设 + 可选会话 | v1.53.0 完整备份；快照/偏好仍可选 v2 |
...
### 代码与架构备注（2026-05-29 第三十四次挖掘）

- **睡眠/唤醒对称**：`sleepTimer.ts` / `wakeTimer.ts` 均支持倒计时与 `start*TimerAtClock`；快照与 Web Audio 斜坡共用模式。
- **下一项候选**：`fullAppBackup` v2（混音快照 + 主题/渐入/常亮等偏好）；Android 后台播放需原生调研。
- **音库**：17 内置轨；避免连续版本仅加音。
- **测试**：`sleepTimer.test.ts`、Studio 抽屉「按时刻停止」UI/读屏测试。
- **版本**：**1.57.0** → 本次 **1.58.0** 睡眠按时刻停止。

### 外部信号

- GitHub Issues：无 open issue（2026-05-29）。
- 同类 App：固定时刻睡眠停止与闹钟对称；wix 睡眠/唤醒均已支持时刻路径。

## 本次选中项

**睡眠定时按时刻（P2）**

- **理由**：v1.57.0 已为唤醒增加按时刻；backlog 明确「23:00 渐弱」缺口；复用 `SleepTimerState`、快照与读屏模式，单 PR 无原生依赖。
- **范围**：`sleepTimer.ts`、`useSleepTimerController`、`StudioPage` 抽屉、`playbackAnnouncement`、版本 **1.58.0**。
