...
| ~~混音状态持久化~~ | 刷新后恢复组合与音量 | — | 本次实现（v1.3.0） |
| **主音量记忆** | 每次打开主音量回到 82% | 已随混音快照一并持久化 | 可与预设功能合并验收 |
...
- 近期 CHANGELOG（v1.2.0）：睡眠定时——**避免重复**。
- 同类 App 常见能力：预设、后台播放、分享配方——与 P1/P2 一致。

## 本次选中项

**混音状态持久化（P0）**

- **理由**：专注/办公用户刷新或误关页面后需恢复声轨组合与各层音量；实现边界清晰（`localStorage` 快照 + 校验 + 自定义轨合并过滤），单 PR 可交付；不依赖 Android 原生。
- **范围**：持久化主音量、立体声宽度、全局速率、各层参数；启动时恢复但保持暂停（遵守自动播放策略）；自定义音轨加载后剔除已删除轨层。

## 历史已完成

| 日期 | 项 | 引用 |
| --- | --- | --- |
| 2026-05-27 | 混音状态持久化 | （本次 PR） |
