...
| ~~音轨搜索/筛选~~ | 14+ 内置 + 多自定义 | 平铺网格 | Studio 搜索框按标题/描述/文件名过滤 |
...
| 读屏 | 播放/加轨/定时/键盘音量 | v1.37.0；滑块拖主音量仍静默 |
| Android 原生 | 后台播放 | 仍为 P1 最大缺口 |

### 代码与架构备注（2026-05-28 第十六次挖掘）

- **Studio**：`filterSoundsByQuery` 纯函数 + 搜索框；空结果 `role="status"` 提示；输入框内不触发 Space/M 等快捷键（已有 `isEditableKeyboardTarget`）。
- **内置声库**：14 轨；下一内容向：工地环境声（P2）。
- **Android**：仍无 foreground service（P1）；分享深链为 P2。
- **落地页**：仍缺 fade-in、wake lock、clear data、搜索框等 v1.28–1.38 功能描述。
- **版本**：当前实现目标 **1.38.0**；下一项宜 **唤醒定时**、**工地环境声** 或 **落地页同步**。

### 外部信号

- GitHub Issues：无 open issue（2026-05-28）。
- 近期 CHANGELOG：v1.37.0 键盘主音量读屏——**避免重复**。
...
- **理由**：14 种内置 + 自定义库增长后平铺网格难定位；纯前端、无 Android 原生依赖；`filterSoundsByQuery` 可单测，UI 改动限于 Studio 主舞台。
- **范围**：`soundSearch.ts` + Studio 搜索框 + vitest；版本 **1.38.0**。

## 历史已完成

| 日期 | 项 | 引用 |
| --- | --- | --- |
| 2026-05-28 | Studio 环境声搜索/筛选 | v1.38.0（本次） |
