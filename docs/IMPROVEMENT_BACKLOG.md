...
| **新内置环境声（续）** | 公路通勤、飞机舱等 | 已有 11 轨（含列车 v1.25.0） | `sfx100v2_loop_highway` 作旅途底噪；或飞机舱 CC0 素材 |
| ~~大文件导入进度~~ | 长播客/长环境录音 | — | v1.16.0 |
| ~~混音 ARIA 实时区域~~ | 读屏知播放/定时状态 | — | 已完成 v1.12.0 |
| ~~落地页功能列表更新~~ | 新用户了解分享链接、快捷键等 | — | 已完成 v1.23.0（v1.25.0 同步十一种内置声） |
...
| 旅行/办公 | 离线 PWA、列车/旅途氛围 | 列车 v1.25.0；公路声仍缺 |
| 弱网/首次加载 | 内置 OGG 偶发失败 | fetch+decode 指数退避重试（v1.8.0） |
| 前庭敏感 | 减少 UI 动效 | 系统「减少动态效果」（v1.10.0） |
| 新用户认知 | 落地页了解 Studio 能力 | v1.25.0 十一种内置声/定时/分享/快捷键 |
...
### 代码与架构备注（2026-05-28 四次挖掘）

- **分层清晰**：`domain/` 纯逻辑、`storage/` 快照、`audio/` 引擎、`AppLayout` 编排；分享深链 `mixerShareUrl` + `useMixerShareDeepLink`。
- **内置声库**：11 轨 CC0 OGG；列车来自 MMRetroArcade TrainLoop1（~2.8s 经 `stream_loop` 延长至 24s）；下一候选公路 `sfx100v2_loop_highway`。
- **睡眠定时**：5–480 分钟自定义（v1.20.0）。
- **键盘**：`studioKeyboard` — Space / M / ± / `?`（v1.21.0）。
- **Android**：Capacitor 8；后台音频仍为最大平台缺口（P1），需原生 foreground service。
- **测试**：domain/hook/StudioPage 覆盖快捷键与深链；`LandingPage.test` 覆盖十一种内置声文案。
- **版本**：当前实现目标 **1.25.0**；下一项宜 **公路旅途环境声**（P2）或 **Android 后台**（P1，大项）。

### 外部信号

- GitHub Issues：无 open issue（2026-05-28）；近期 PR 以每日小功能为主。
- 近期 CHANGELOG：v1.24.0 咖啡馆——**避免重复**。
- 同类 App：列车白噪常见；Web 功能已较完整，内容与 Android 原生后台为主要缺口。

## 本次选中项

**内置列车环境声（P2）**

- **理由**：旅行/通勤场景在 backlog 中列为缺口；CC0 TrainLoop1 可单 PR 完成（下载脚本 + `train.ogg` + 目录/落地页）；与 v1.24.0 咖啡馆形成「办公+旅途」内容补充。
- **范围**：`train.ogg`、`sounds.ts`、`download-builtin-sounds.sh`、ATTRIBUTION、LandingPage；版本 **1.25.0**。

## 历史已完成

| 日期 | 项 | 引用 |
| --- | --- | --- |
| 2026-05-28 | 内置列车环境声 | v1.25.0（本次） |
