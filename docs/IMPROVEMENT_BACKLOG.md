...
| ~~场景预设同名覆盖~~ | 12 槽满后无法更新常用搭配 | 仅可删后重存 | 已完成 v1.54.0 |
| ~~场景预设复制~~ | 在「专注」基础上微调另存 | 只能覆盖或重命名 | 已完成 v1.60.0 |
...
| **新内置环境声（续）** | 雨棚、鸟鸣、风声等 | 已有 17 轨 | 评估 CC0；避免连续版本仅加音 |
...
| ~~完整备份（单文件）~~ | 换机一次导入音频+预设 | v1.51–1.52 需两个 JSON | 已完成 v1.53.0 `wix-full-backup` |
...
| ~~预设重命名~~ | 修正错字而不改混音 | 仅删/新建 | 已完成 v1.55.0 |
| ~~定时渐出/渐强 Web Audio 斜坡~~ | 睡眠/唤醒更顺滑 | — | 已完成 v1.56.0 |
| ~~时钟点唤醒~~ | 「7:30 叫醒」 | 仅「N 分钟后」 | 已完成 v1.57.0 `startWakeTimerAtClock` |
| ~~睡眠定时按时刻~~ | 23:00 自动渐弱 | 仅倒计时分钟 | 已完成 v1.58.0 `startSleepTimerAtClock` |
| ~~完整备份含混音快照与偏好~~ | 换机后立刻恢复当前会话 | v1.53 仅音频+预设 | 已完成 v1.59.0 `wix-full-backup` v2 |

### 体验场景与缺口（摘要）

| 场景 | 典型需求 | 现状 |
| --- | --- | --- |
| 睡眠 | 定时、渐出、按时刻停止、读屏、常亮 | v1.58.0 按时刻停止；v1.34–1.35 倒计时与后台恢复 |
| 午睡/叫醒 | 倒计时或固定时刻渐强起播 | v1.41.0 唤醒；v1.57.0 按时刻叫醒 |
| 冥想/放松 | 自然水声、森林氛围 | v1.50.0 溪流 |
| 专注/办公 | 预设、办公室氛围、键盘 | v1.60.0 预设复制；v1.54–1.55 覆盖与重命名 |
| 哄娃/屏蔽突发声 | 白噪音底噪 | v1.47.0 内置白噪音 |
| 分享/社交 | 发链接好友一键同款混音 | Web/PWA + Android v1.44–1.45 |
| 换机/备份 | 迁移自定义音频 + 场景预设 + 当前混音与偏好 | v1.59.0 完整备份 v2 |
| 居家/城市 | 屏蔽装修与街道施工 | v1.39.0 工地环境声 |
| 多任务 | 切标签/后台后仍听到混音 | Web/PWA v1.35.0；Android 仍待 P1 |
| 隐私/换机 | 一键清除本机数据 | v1.31.0 |
| 弱网 | 内置 OGG 偶发失败 | 自动重试 + 按轨重试 v1.28.0 |
| 读屏 | 播放/加轨/定时/键盘与滑块音量 | 睡眠与唤醒按时刻均已播报 |
| 锁屏/PWA | 睡眠与唤醒倒计时 | v1.46.0 Media Session 副标题 |
| Android 原生 | 后台播放 | 仍为 P1 最大缺口 |

### 代码与架构备注（2026-05-29 第三十六次挖掘）

- **预设**：`mixerPresets.ts` 支持保存/覆盖/重命名/复制；复制自动命名 `原名 副本`（冲突时递增）。
- **完整备份 v2**：`fullAppBackup.ts` 含混音快照与偏好；不含活动定时快照（时间敏感）。
- **下一项候选**：Android 后台播放（P1，需 Foreground Service 调研）；落地页同步 v1.57–1.60 能力摘要。
- **音库**：17 内置轨；避免连续版本仅加音。
- **版本**：**1.59.0** → 本次 **1.60.0** 场景预设复制。

### 外部信号

- GitHub Issues：无 open issue（2026-05-29）。
- 同类 App：预设变体常通过「复制预设」而非覆盖；wix v1.60.0 已对齐。

## 本次选中项

**场景预设复制（P1）**

- **理由**：v1.54–1.55 已支持覆盖与重命名，但用户想在保留「专注」的同时另存「专注·雨声」变体时只能删槽或覆盖；单 PR 扩展现有 `mixerPresets` + 抽屉按钮，无原生依赖。
- **范围**：`mixerPresets.ts`、`AppLayout`、`StudioPage`、测试、版本 **1.60.0**。

## 历史已完成

| 日期 | 项 | 引用 |
| --- | --- | --- |
| 2026-05-29 | 场景预设复制 | v1.60.0 |
| 2026-05-29 | 完整备份 v2（混音快照 + 偏好） | v1.59.0 |
| 2026-05-29 | 睡眠定时按时刻 | v1.58.0 |
| 2026-05-29 | 时钟点唤醒 | v1.57.0 |
