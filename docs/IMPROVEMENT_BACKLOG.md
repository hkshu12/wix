...
**场景预设备份导出/导入（P2 → 换机/重装，与 v1.51 自定义音频备份配套）**

- **理由**：v1.51.0 已支持自定义音轨 JSON 备份，但用户保存的「专注/睡眠」场景预设仍无法迁移；单 PR 扩展设置页备份能力，不碰 Android 原生；比再增一条 CC0 环境声对用户换机价值更高。
- **范围**：`mixerPresetsBackup.ts`、`replaceMixerPresets`、`SettingsPage` 预设区、版本 **1.52.0**。
