...
| ~~可配置睡眠渐出时长~~ | 哄娃需快速停、睡眠需长渐弱 | 固定 30 秒 | 10/30/60/120 秒预设 + localStorage 记忆 |
| **设置页清除全部数据** | 换机/隐私重置 | 仅说明存储，无一键重置 | 清除快照、预设、IndexedDB、睡眠定时 |
| **播放渐入** | 睡眠/婴儿场景避免突兀起播 | 起播约 40ms 斜坡 | 可选 2–10 秒主音量渐入 |
| **睡眠定时读屏播报** | 开始/取消/结束定时 | 仅抽屉内 `aria-live` 倒计时 | 扩展 `playbackAnnouncement` |
...
### 代码与架构备注（2026-05-28 八次挖掘）

- **分层清晰**：`domain/` 纯逻辑、`storage/` 快照与偏好、`audio/` 引擎、`AppLayout` 编排。
- **睡眠定时**：`startSleepTimer(now, minutes, fadeSeconds)` 已用于控制器；渐出偏好 `wix.sleepTimerFadeSeconds`（v1.30.0）。
- **音频引擎**：`AudioEngine.startLayer` 仅 ~40ms 增益斜坡；长渐入仍为 P2。
- **内置声库**：13 轨 CC0 OGG；下一内容向：办公室 HVAC。
- **Android**：`AndroidManifest` 无 foreground service；最大平台缺口（P1）。
- **版本**：当前 **1.30.0**；下一项宜 **办公室环境声**（P2）或 **设置页清除数据**（P2）或 **播放渐入**（P2）。

### 外部信号

- GitHub Issues：无 open issue（2026-05-28）；近期以每日小功能与内置声为主。
- 近期 CHANGELOG：v1.29.0 Android 更新修复——**避免重复**。
- 同类 App：可配置渐出已补齐；办公室 HVAC、Android 原生后台、播放渐入仍常见缺口。

## 本次选中项

**可配置睡眠渐出时长（P2）**

- **理由**：`startSleepTimer` 已支持 `fadeSeconds` 参数但 UI/偏好未接；哄娃（10 秒）与睡眠（60–120 秒）场景差异大；单 PR、无原生依赖。
- **范围**：`sleepTimer` 域常量、`sleepTimerPreferences` 存储、控制器、`StudioPage` 渐出预设、测试；版本 **1.30.0**。

## 历史已完成

| 日期 | 项 | 引用 |
| --- | --- | --- |
| 2026-05-28 | 可配置睡眠渐出时长 | v1.30.0（本次） |
